...
with st.spinner("Loading Model....(100%)"):
    model_phobert = load_model_phobert()
...
# sentences = [' '.join(rdrsegmenter.word_segment(sentence)) for sentence in sentences]

if st.button('Analyze'):
    if model_name == 'PhoBert':
        all_entities, texts = predict_phobert(sentences, model_phobert)
        st.write('*Ner Tags:*')
    ...
    if model_name == 'biLSTM':
        res, texts = predict_bilstm(sentences, model_bilstm)
        print("======" , texts)
        def show(pred, text):
            text = text.split()
